...
if (!$conn) {
	echo "<p>Database connection failure</p>";
		...
		$make = trim($_POST["STUDENTID"]);
		$made = trim($_POST["ATTEMPT"]);
		$GRADE = trim($_POST["GRADE"]);
		
		$query = "UPDATE attempts SET Score = '$GRADE' WHERE StudentID = '$make' and Attempt ='$made'";
		$result = mysqli_query($conn, $query);
	if (!$result) {   
		echo "<p> Something is wrong with ", $query, "</p>";
	} else {
		$query = "SELECT * FROM attempts WHERE StudentID = '$make' and Attempt = '$made'";
		$result = mysqli_query($conn, $query);
